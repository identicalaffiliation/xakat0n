...
|---|---|---|
| `POST /auth/login` | — | Находит или создаёт пользователя по username, выдаёт `session_token`. Единственный эндпоинт без авторизации |

### `internal/items`

| Эндпоинт | advance | Что делает |
|---|---|---|
| `GET /items` | — | Каталог. Отдаёт `is_limited` и `sold_out`, число свободных мест — намеренно нет |
