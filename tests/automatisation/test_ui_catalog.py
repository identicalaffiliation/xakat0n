...
def test_seed_items_and_limited_badge_visible(catalog: Page) -> None:
    ...
    catalog.get_by_text("Компьютерный стол", exact=True).wait_for()
...
def test_category_filter_shows_only_matching(catalog: Page) -> None:
    catalog.get_by_role("button", name="Спорт").click()
    catalog.get_by_text(SEED_REGULAR, exact=True).wait_for()
    catalog.get_by_text("Турник настенный", exact=True).wait_for()
    assert catalog.get_by_text(SEED_LIMITED, exact=True).count() == 0
